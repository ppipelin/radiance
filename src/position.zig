    ...
    pub fn scoreMoves(self: Position, list: []Move, comptime flag: GenerationType, scores: []Value) void {
        for (list, 0..) |move, i| {
            ...
            if (flag == .capture or flag == .all and move.isCapture()) {
                ...
                if (search.seeGreaterEqual(self, move, -100))
                    scores[i] += 40 * @as(Value, @intFromBool((self.state.check_square[self.state.turn.invert().index()][from_piece.index()] & move.getTo().sqToBB()) > 0));
            }
