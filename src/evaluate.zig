...
pub fn evaluateShannon(pos: position.Position) types.Value {
    if (pos.state.turn.isWhite()) {
        return evaluateShannonColor(pos, types.Color.white) - evaluateShannonColor(pos, types.Color.black);
    } else {
        return evaluateShannonColor(pos, types.Color.black) - evaluateShannonColor(pos, types.Color.white);
    }
}
