...
pub fn updateHistory(turn: Color, move: Move, bonus: Value) void {
    const clamped_bonus: Value = std.math.clamp(bonus, -max_history, max_history);
    const abs_clamped_bonus: Value = @intCast(@abs(clamped_bonus));
    const last_value: Value = history[turn.index()][move.getFromTo()];

    const tapered: Value = @truncate(@divTrunc(@as(i64, last_value) * @as(i64, abs_clamped_bonus), max_history));
