...
pub var passed_pawn: [types.board_size2][types.Color.nb()]Bitboard = std.mem.zeroes([types.board_size2][types.Color.nb()]Bitboard);
...
fn initPassedPawn() void {
    ...
    while (sq != Square.a8) : (sq = sq.inc().*) {
        passed_pawn[sq.index()][types.Color.white.index()] = filterPassedPawn(sq, types.Color.white);
        passed_pawn[sq.index()][types.Color.black.index()] = filterPassedPawn(sq, types.Color.black);
    }
}
...
pub const passedPawnTable = [8]types.Value { 0, 15, 15, 25, 40, 60, 70 };
