...
pub fn initOptions(allocator: std.mem.Allocator, options: *std.StringArrayHashMapUnmanaged(Option)) !void {
    ...
    try options.put(allocator, "Evaluation", try Option.initCombo(allocator, "NNUE var PSQ var PSQ var Shannon", "NNUE"));
...
pub fn loop(io: std.Io, allocator: std.mem.Allocator, stdin: *std.Io.Reader, stdout: *std.Io.Writer, args_iter_: ?*std.process.Args.Iterator) !void {
    ...
    if (args_iter_) |args_iter|
        _ = args_iter.next(); // Discard program path

    while (true) {
        // Prior to parsing stdin, parses args_iter
        const tline = if (args_iter_) |args_iter|
            args_iter.next() orelse
                std.mem.trim(u8, try stdin.takeDelimiter('\n') orelse break, " \r")
        else
            std.mem.trim(u8, try stdin.takeDelimiter('\n') orelse break, " \r");
...
fn cmd_genfens(io: std.Io, stdout: *std.Io.Writer, tokens: anytype) !void {
    if (tokens.next()) |token_value| {
        ...
        const depth: u32 = @ceil(@max(8, 2 + @log10(@as(f32, @floatFromInt(N)))));
