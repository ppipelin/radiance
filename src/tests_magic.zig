...
test "MovegenAccessions" {
    ...
    for (0..iterations) |_| {
        ...
        while (sq != types.Square.none) : (sq = sq.inc().*) {
            ...
            std.debug.print("{d}\n", .{moves_bishop_blockers});

            var tmp: std.ArrayListUnmanaged(types.Bitboard) = .empty;
            defer tmp.deinit(allocator);
            tmp.append(allocator, 0) catch unreachable;
            tables.computeBlockers(tables.moves_bishop_mask[sq.index()], &tmp, allocator);

            std.debug.print("{d}\n", .{tmp.items});
            for (moves_bishop_blockers) |blockers| {
                std.debug.print("blockers {}", .{blockers});
...
test "MovegenAccessionsOld" {
    ...
    for (0..iterations) |_| {
        var sq: types.Square = types.Square.a1;
        while (sq != types.Square.a2) : (sq = sq.inc().*) {
            ...
            for (moves_rook_blockers.items) |blockers| {
                ...
                std.debug.print("{}\n", .{tables.moves_rook_mask[sq.index()] & blockers});
                _ = tables.moves_rook[sq.index()].get(tables.moves_rook_mask[sq.index()] & blockers) orelse unreachable;
            }
        }
    }
