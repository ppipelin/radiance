...
pub fn initMagic() void {
    ...
    while (sq != types.Square.none) : (sq = sq.inc().*) {
        ...
        const blockers_bishop = tables.computeBlockersComptime(magics_bishop[sq.index()].mask);

        for (blockers_bishop) |blocker| {
        ...
        const blockers_rook = tables.computeBlockersComptime(magics_rook[sq.index()].mask);
        for (blockers_rook) |blocker| {
...
fn generateMagic(magic_out: *Magic, ptr: [*]Bitboard, sq: types.Square, mask: Bitboard, getAttacks: fn (types.Square, Bitboard) Bitboard, shift: u8, prng: *std.Random.DefaultPrng) void {
    const blockers = tables.computeBlockersComptime(mask);
