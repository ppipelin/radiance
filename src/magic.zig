...
const magic_numbers: [types.board_size2 * 2]u64 = [_]u64{
    ...
    0x100c0409c50d0090, 0x3002020fc9a80080, 0x816a221104010,    0x210288505980600,  0x2004202008504014, 0x20410204011040,   0x4020201382149c44, 0x648222403d010,
    ...
    0x842220c83004600,  0x40810a4589220200, 0x1008200088100080, 0x100a00412200,     0x20000c200a006600, 0x1000040003070100, 0x8039020810c400,   0x90202040904820,
    ...
    for (0..iterations) |_| {
        ...
        while (sq != .none) : (sq = sq.inc().*) {
            ...
            if (magic_b.shift > magics_bishop[sq.index()].shift) {
                std.debug.print("found better bishop magic for sq {}, from {} to {}\n", .{ sq, magics_bishop[sq.index()].shift, magic_b.shift });
            ...
            if (magic_r.shift > magics_rook[sq.index()].shift) {
                std.debug.print("found better rook magic for sq {}, from {} to {}\n", .{ sq, magics_rook[sq.index()].shift, magic_r.shift });
